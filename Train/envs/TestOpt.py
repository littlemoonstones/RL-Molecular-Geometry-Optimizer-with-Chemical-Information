...
class MoleculeV20(gym.Env):
    ...
    def __init__(self,
        ...
        super().__init__()
        print("Using TestOpt env", self.__class__.__name__)
        ...
        self.Encode: EncodeClass = self.encode_class(geometry, smile)
    ...
    def createGeometry(self, smile: str, coord: np.ndarray, out_dir=".") -> Tuple[List[str], Geometry]:
        ...
        # geometry.set_calculator(MMFF(smile=smile, seed=_seed))
        # geometry.set_calculator(XTB(quiet=True, out_dir=out_dir))
        # geometry.set_calculator(MOPAC("PM7", out_dir=out_dir))
        geometry.set_calculator(MOPAC("PM6", out_dir=out_dir))
        print("Using calculator:", geometry.calculator.__class__.__name__)
    ...
    def step(self, action: np.ndarray):
        ...
        try:
            ...
            state = self.state
            if error is True or is_converged is True:
    ...
    def reset(self, smile: str=None, coord=None):
        ...
        self.tmp_folder = tempfile.TemporaryDirectory(prefix="tmp-", dir=".")
        self.iter = 0
       ...
       # initial geoemtry force needs larger then threshold
        atoms, tmp_geometry = self.createGeometry(self.smile, coord, self.tmp_folder.name)
