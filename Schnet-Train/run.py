...
from lib.ModelType import ModelMetaData, SchNetMetaData
import configparser
import envs
...
if __name__ == "__main__":
    ...
    assert type(n_head) == int
    assert type(hidden_size) == int
    assert type(n_atom_basis) == int
    assert type(n_filters) == int
    assert type(n_interactions) == int
    ...
    act_net = model_factory.getActor().to(device)

    crt_net = model_factory.getCritic().to(device)

    twinq_net = model_factory.getTwinQ().to(device)
    ...
    writer = SummaryWriter(Path("runs", "RL-", version, "seed-" + str(seed)))
    ...
    with ptan.common.utils.RewardTracker(writer) as tracker:
        with ptan.common.utils.TBMeanTracker(
            ...
            while frame_idx < TOTAL_FRAMES:
                ...
                states, actions_v, ref_vals_v, ref_q_v = \
                ...
                # train TwinQ
                twinq_opt.zero_grad()
                q1_v, q2_v = twinq_net(states, actions_v)
                ...
                # Actor
                act_opt.zero_grad()
                acts_v = act_net(states)
                q_out_v, _ = twinq_net(states, acts_v)

                act_loss = -q_out_v.mean()
                act_loss.backward()
                # print("act_net:")
                # printModelGrad(act_net)
