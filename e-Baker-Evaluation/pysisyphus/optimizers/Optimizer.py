...
CONV_THRESHS = {
    ...
    "paper": (4.5e-4, 3.0e-4, 1.8e-3, 1.2e-3),
    # Dummy thresholds
    "never": (2.0e-6, 1.0e-6, 6.0e-6, 4.0e-6),
}
Thresh = Literal["gau_loose", "gau", "gau_tight", "gau_vtight", "baker", "never", "paper"]
...
class Optimizer(metaclass=abc.ABCMeta):
    def __init__(
        ...
        # debug
        self.cart_steps: List[np.ndarray] = list()
        self.cart_forces: List[np.ndarray] = list()
    ...
    def final_summary(self):
        ...
        int_str = ""
        # debug: add redund
        if self.geometry.coord_type not in ("cart", "cartesian", "mwcartesian", "redund"):
    ...
    def run(self):
        ...
        for self.cur_cycle in range(self.last_cycle, self.max_cycles):
            ...
            # _geometry = Geometry(
            #     self.geometry.atoms, 
            #     copy.deepcopy(self.geometry.cart_coords), 
            #     # self.geometry.cart_coords.copy(), 
            #     coord_type="redund",
            #     coord_kwargs={
            #         "typed_prims": copy.deepcopy(self.geometry.internal.typed_prims)
            #     }
            # )
            _calculator = self.geometry.calculator
            self.geometry.set_calculator(None)
            _geometry = copy.deepcopy(self.geometry)
            self.geometry.set_calculator(_calculator)
            # print("equal", _geometry.coords - self.geometry.coords)
            # print("len new_coords", len(new_coords))
            # print("len _geometry.coords", len(_geometry.coords))
            # print("len geometry.coords", len(self.geometry.coords))
            # print(step)
            _geometry.coords = new_coords
            _cart_step = _geometry.cart_coords - self.geometry.cart_coords
            self.cart_steps.append(_cart_step)
            del _geometry
            ...
            try:
                ...
                # print("max rad step", np.abs(self.steps[-1]).max())

                # debug
                # self.cart_steps[-1] = (self.geometry.cart_coords - self.cart_coords[-1])
                # print("size of cart_steps: ", len(self.cart_steps))
                # print("size of steps: ", len(self.steps))
