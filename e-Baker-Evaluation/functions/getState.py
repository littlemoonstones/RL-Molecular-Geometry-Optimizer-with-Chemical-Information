...
class LogGradienttState(State):
    def __init__(self, geometry: Geometry) -> None:
        # print("TESTING MODIFIED WARNING")
        super().__init__(geometry)
    
    # testing
    # def getState(self):
    #     g_sign, g_array = splitLog(self.geo.gradient*10)
    #     return np.hstack([
    #         g_sign[:, None],
    #         g_array[:, None],
    #     ]) 
    ...
    # def getState(self):
    #     factor = 1
    #     if np.abs(self.geo.gradient).max() < 1e-3:
    #         factor = 10
    #     g_sign, g_array = splitLog(self.geo.gradient*factor)
