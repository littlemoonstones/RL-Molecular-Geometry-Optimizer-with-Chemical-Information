...
class StandardizedActionAutoLinearFactorFixed2(StandardizedActionFactor):
    def __init__(self, geometry: Geometry) -> None:
        super().__init__(geometry)
    ...
    def getAction(self, action: np.ndarray, gradients: np.ndarray):
        ...
        raw_a, raw_b = action[:, 0], action[:, 1]
        a = self.transform(raw_a, 1.1, 0.8)
        b = self.transform(raw_b, 2, -2)
        exp = log_grad * a + b
...
class StandardizedActionAutoLinearFactorFixed3(StandardizedActionFactor):
    def __init__(self, geometry: Geometry) -> None:
        super().__init__(geometry)
    ...
    def getAction(self, action: np.ndarray, gradients: np.ndarray):
        ...
        raw_a, raw_b = action[:, 0], action[:, 1]
        a = self.transform(raw_a, 1.0, 0.8)
        b = self.transform(raw_b, 3, -2)
        exp = log_grad * a + b
